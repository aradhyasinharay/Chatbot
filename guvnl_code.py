...
def handle_plant_info(original_message):
    try:
        ...
        # Normalize mapping to match field names in JSON
        plant_field_map = {
            "plf": "PLF",
            "plant load factor": "PLF",
            "paf": "PAF",
            "plant availability factor": "PLF",
        ...
        normalized_map = {normalize(k): v for k, v in plant_field_map.items()}

        # Check if a field is being requested
        for keyword, field_key in plant_field_map.items():
            if keyword in message:
                # Check for plant name (e.g., "plf for Waste to Energy")
                match = re.search(rf"{keyword}.*(?:for|of) (.+)", message)
                if match:
                    plant_query = normalize(match.group(1))
                    for plant in all_plants:
                        if normalize(plant["name"]) == plant_query:
                            value = plant.get(field_key, 'N/A')
                            return f"{field_key} for {plant['name']}: {value}"
                    return f"No plant found matching '{match.group(1)}'."
                else:
                    # If no specific plant, list field values for all
                    lines = []
                    for plant in all_plants:
                        value = plant.get(field_key, 'N/A')
                        lines.append(f"{plant['name']} ({field_key}): {value}")
                    return f"{field_key} values for all plants:\n" + "\n".join(lines)

        # If user just wants general plant details
        lines = []
        for plant in all_plants:
            lines.append(f"{plant['name']} - Rated Capacity: {plant.get('Rated_Capacity', 'N/A')} MW, Type: {plant.get('Type', 'N/A')}")
        return "List of Must Run Plants:\n" + "\n".join(lines)
...
def get_response(user_input):
    ...
    if any(k in matched_keywords for k in ['plf', 'paf', 'variable cost', 'aux consumption',
                                           'max power', 'min power', 'rated capacity',
                                           'technical minimum', 'type', 'maximum power', 'minimum power',
                                           'auxiliary consumption', 'plant load factor', 'plant availability factor'
                                           'aux usage', 'auxiliary usage', "var cost"]):
        return handle_plant_info(user_input)
